-- 02_analytic_cohort.sql  (REBUILD v2, Aug 2026)
...
mm AS (
  SELECT subject_id, study_id, ecg_time, cart_id, bandwidth, filtering,
    ...
    -- NOTE: "revers" is intended for lead reversal but will also match "reversible". Exclusion
    -- counts are reported by the cohort-flow diagnostic below; see docs/DECISIONS_AND_RATIONALE.md.
...
ecg AS (
  SELECT r.subject_id, r.study_id, r.ecg_time, r.path AS ecg_path,
         mm.ventricular_rate, mm.pr_interval, mm.qrs_duration, mm.qt_corrected,
         mm.paced, mm.poor_quality, mm.has_valid_meas,
         mm.cart_id, mm.bandwidth, mm.filtering
...
paired AS (
  SELECT e.*, p.gender, p.anchor_age + (EXTRACT(YEAR FROM e.ecg_time) - p.anchor_year) AS age_at_ecg,
         l.* EXCEPT(subject_id),
         DATETIME_DIFF(l.echo_dt, e.ecg_time, DAY) AS ecg_to_echo_days,
...
SELECT *,
       (recency_rank=1) AS most_recent_per_patient,
       (pr_interval IS NULL) AS pr_missing,                       -- released rule fills RAW 0 pre-scaling
